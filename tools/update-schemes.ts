...
const unofficial = await import('./../lib/unofficial.json', { with: { type: "json" } });
...
get.concat('http://www.iana.org/assignments/uri-schemes/uri-schemes.xml', function (err: any, res: any, data: { toString: () => any; }) {
  if (err) {
    throw err;
  }
  ...
  Object.keys(sections).forEach(function (key) {
    ...
    fs.readFile(fileName, 'utf8', function (err: any, data: string) {
      ...
      if (diff.added.length > 0) {
        diffLog.push('\t' + chalk.green('Added') + ': ' + diff.added.map(function (record: { scheme: any; }) { return record.scheme; }).join(', '));
      }

      if (diff.removed.length > 0) {
        diffLog.push('\t' + chalk.red('Removed') + ': ' + diff.removed.map(function (record: { scheme: any; }) { return record.scheme; }).join(', '));
      }
      ...
      if (diff.common.length > 0) {
        ...
        const updated = diff.common.filter(function (record: { scheme: string | number; }) {
          return !deepEqual(oldMap[record.scheme], newMap[record.scheme]);
        });

        if (updated.length > 0) {
          diffLog.push('\t' + chalk.yellow('Updated') + ': ' + updated.map(function (record: { scheme: any; }) { return record.scheme; }).join(', '));
        }
      }
      ...
      fs.writeFile(fileName, JSON.stringify(newRecords, undefined, 2), function (err: any) {
        if (err) {
          throw err;
        }
